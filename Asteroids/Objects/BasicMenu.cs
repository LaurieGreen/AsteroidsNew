...
namespace Asteroids
{
    public class BasicMenu
    {
        ...
        public BasicMenu(SpriteFont small_font, SpriteFont medium_font, SpriteFont large_font, String title,  List<String> choiceList)
        {
...
#pragma warning restore CS0162 // Unreachable code detected
        ...
        public void Draw(SpriteBatch spriteBatch, float width, float height, Color color, GameTime gameTime)
        {
            ...
            for (int i = 0; i < choiceList.Count(); i++)
            {
                ...
                spriteBatch.DrawString(small_font, choiceList[i], new Vector2(width / 2, (height /8)* 6 + ((small_font.MeasureString(choiceList[i]).Y)*i)), choiceColor, 0, new Vector2(small_font.MeasureString(choiceList[i]).Length() / 2, small_font.MeasureString(choiceList[i]).Y / 2), scale, SpriteEffects.None, 0);
            }
            spriteBatch.End();
