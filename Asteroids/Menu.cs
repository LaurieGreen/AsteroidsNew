...
namespace Asteroids
{
    public class Menu
    {
        ...
        public void Draw(SpriteBatch spriteBatch, float width, float height, Color color, GameTime gameTime)
        {
            ...
            spriteBatch.DrawString(medium_font, title, new Vector2(width / 2 - (medium_font.MeasureString(title).Length() / 2), height / 8 + (2 * medium_font.MeasureString(title).Y)), Color.White);
            for (int i = 0; i < choiceList.Count(); i++)
            {
                ...
                {
                    choiceColor = new Color(0, 204, 0);//yellow
                ...
                spriteBatch.DrawString(small_font, choiceList[i], new Vector2(width / 2, (height /8)* 5 + ((small_font.MeasureString(choiceList[i]).Y)*i)), choiceColor, 0, new Vector2(small_font.MeasureString(choiceList[i]).Length() / 2,0), scale, SpriteEffects.None, 0);
            }
            spriteBatch.End();
